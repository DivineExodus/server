...
public class VolcanicArea extends InstanceContainer implements World {
    public static final VolcanicArea INSTANCE;
    private static final DimensionType DIMENSION;
    private static final Biome BIOME;

    static {
        DIMENSION = DimensionType.builder(NamespaceID.from("divineexodus:volcanic_area"))
                .build();
        MinecraftServer.getDimensionTypeManager().addDimension(DIMENSION);
        BIOME = Biome.builder()
                .name(NamespaceID.from("divineexodus:volcanic_area"))
                .build();
        MinecraftServer.getBiomeManager().addBiome(BIOME);
        INSTANCE = new VolcanicArea();
        MinecraftServer.getInstanceManager().registerInstance(INSTANCE);
    }

    VolcanicArea() {
        super(UUID.nameUUIDFromBytes("VOLCANIC_AREA".getBytes()), DIMENSION);
