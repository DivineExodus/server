...
public class VolcanoExternal extends InstanceContainer implements World {
    public static final VolcanoExternal INSTANCE;
    private static final DimensionType DIMENSION;
    private static final Biome BIOME;

    static {
        DIMENSION = DimensionType.builder(NamespaceID.from("divineexodus:volcano_external"))
                .build();
        MinecraftServer.getDimensionTypeManager().addDimension(DIMENSION);
        BIOME = Biome.builder()
                .name(NamespaceID.from("divineexodus:volcano_external"))
                .build();
        MinecraftServer.getBiomeManager().addBiome(BIOME);
        INSTANCE = new VolcanoExternal();
        MinecraftServer.getInstanceManager().registerInstance(INSTANCE);
    }

    VolcanoExternal() {
        super(UUID.nameUUIDFromBytes("VOLCANO-EXTERNAL".getBytes()), DIMENSION);
