...
public class Dry extends InstanceContainer implements World {
    public static final Dry INSTANCE;
    private static final DimensionType DIMENSION;
    private static final Biome BIOME;

    static {
        DIMENSION = DimensionType.builder(NamespaceID.from("divineexodus:dry"))
                .build();
        MinecraftServer.getDimensionTypeManager().addDimension(DIMENSION);
        BIOME = Biome.builder()
                .name(NamespaceID.from("divineexodus:dry"))
                .build();
    ...
    Dry() {
        super(UUID.nameUUIDFromBytes("DRY".getBytes()), DIMENSION);
