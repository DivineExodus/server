...
public class Desert extends InstanceContainer implements World {
    public static final Desert INSTANCE;
    private static final DimensionType DIMENSION;
    private static final Biome BIOME;

    static {
        DIMENSION = DimensionType.builder(NamespaceID.from("divineexodus:desert"))
                .build();
        MinecraftServer.getDimensionTypeManager().addDimension(DIMENSION);
        BIOME = Biome.builder()
                .name(NamespaceID.from("divineexodus:desert"))
                .build();
    ...
    Desert() {
        super(UUID.nameUUIDFromBytes("DESERT".getBytes()), DIMENSION);
