...
public class Mountain extends InstanceContainer implements World {
    public static final Mountain INSTANCE;
    private static final DimensionType DIMENSION;
    private static final Biome BIOME;

    static {
        DIMENSION = DimensionType.builder(NamespaceID.from("divineexodus:moutain"))
                .build();
        MinecraftServer.getDimensionTypeManager().addDimension(DIMENSION);
        BIOME = Biome.builder()
                .name(NamespaceID.from("divineexodus:moutain"))
                .build();
    ...
    Mountain() {
        super(UUID.nameUUIDFromBytes("MOUNTAIN".getBytes()), DIMENSION);
